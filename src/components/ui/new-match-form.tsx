...
const NewMatchForm = ({
  ...
  const handleSelectAll = () => {
    ...
    const allPlayerIds = filteredPlayers.map((player) => player.id.toString());
    setValue("playerIds", allPlayerIds);
  };
  ...
  const allFilteredSelected = filteredPlayers && filteredPlayers.length > 0 &&
      filteredPlayers.every((player) => selectedPlayerIds.includes(player.id.toString()));
  ...
  const getBothTeamsPlayers = async (
    ...
    const homeRes = await get(`/admin/teams/active/${homeTeamId}/players`);
    console.log(homeRes)
    ...
    const awayRes = await get(`/admin/teams/active/${awayTeamId}/players`);
              ...
              {selectedFixture && (
                  <div className="space-y-4">
                      ...
                      <div className="flex items-center gap-4">
                        <div className="flex items-center gap-2">
                          ...
                          <Label htmlFor="select-all" className="cursor-pointer">
                            Select All ({filteredPlayers?.length || 0})
                          </Label>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                      {/* Available Players - Takes up 2 columns */}
                            ...
                            return (
                                ...
                                >
                                  <div className="flex-1 min-w-0">
                                    ...
                                    <div className="text-sm text-muted-foreground">
                                      {player.position} • Rating: {player.player_rating}
                                    </div>
                                    {player.team?.name && (
                                    ...
                                    <Badge className={`text-xs mt-1 ${teamBadgeColor}`}>
                                      {teamName}
                                    </Badge>
                                  </div>
                                  <Button
                      ...
                      {/* Selected Players - Takes up 1 column */}
                      <div className="lg:col-span-1">
                        <h4 className="font-medium text-foreground mb-3">
                          Selected ({getValues("playerIds")?.length || 0})
                        </h4>
